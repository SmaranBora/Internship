...
def predict_img():
    if request.method == "POST":
        if 'file' in request.files:
            ...
            if file_extension == 'jpg':
                ...
                model = YOLO('best.pt')
                roi_x1, roi_y1 = 0, 100  # Top-left corner coordinates
                roi_x2, roi_y2 = 1000, 500  # Bottom-right corner coordinates

                # Set the frame skip interval (process every nth frame)
                frame_skip = 15  # Adjust as needed based on your requirements
    ...
    # Crop the frame to the ROI
                    ...
                    results = model.predict(frame, save=True,show=True)  #working
                    print(results)
                    cv2.waitKey(1)
